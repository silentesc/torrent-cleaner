...
use crate::{
    config::Config,
    jobs::{handle_forgotten::HandleForgotten, handle_not_working::HandleNotWorking},
...
impl JobManager {
    pub fn new(config: Config, torrent_manager: Arc<TorrentManager>, media_path: String) -> Self {
        Self { config, torrent_manager, media_path, job_lock: Arc::new(Mutex::new(())) }
    }
    ...
    pub fn setup(&self) {
        ...
        if self.config.jobs().handle_not_working().interval_hours() != -1 {
            ...
            tokio::spawn(async move {
                Logger::info(format!("[job_manager] Set up handle_not_working, next run in {} hours", config_for_handle_not_working.jobs().handle_not_working().interval_hours()).as_str());
                ...
                loop {
                    {
                        ...
                        Logger::info(format!("[job_manager] handle_not_working finished, next run in {} hours", config_for_handle_not_working.jobs().handle_not_working().interval_hours()).as_str());
                    }
                    // Sleep
                    sleep(Duration::from_hours(config_for_handle_not_working.jobs().handle_not_working().interval_hours() as u64)).await;
                }
